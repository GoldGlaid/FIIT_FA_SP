...
big_int &big_int::operator&=(const big_int &other) & {
    ...
    // если нехватает то дополняем А
    _digits.resize(max_size, 0);
    // по ячеечно сравниваем (слева направо)
    for (size_t i = 0; i < max_size; ++i) {
        // для B у нас есть условие (чтобы не выйти за границы)
...
big_int &big_int::operator<<=(size_t shift) & {
    ...
    //shift = 12
    // base = 10
    size_t int_shift = shift / (8 * sizeof(unsigned int)); // 1
    size_t bit_shift = shift % (8 * sizeof(unsigned int)); // 2
    if (int_shift > 0) {
...
big_int &big_int::operator>>=(size_t shift) & {
    ...
    //Работаем с [3][1] (наоборот)
    // делим каждую ячейку с сохранением остатков в следующей (из старшего в младший)
    if (bit_shift > 0) {
        unsigned long long carry = 0;
        for (int i = static_cast<int>(_digits.size()) - 1; i >= 0; --i) {
            // увеличиваем на базу остаток из предыдущего старшего разряда и складываем с текущим числом
            unsigned long long value = (carry << (8 * sizeof(unsigned int))) | _digits[i];
            _digits[i] = static_cast<unsigned int>(value >> bit_shift);
            //маска для выявления потерянных бит из value при смещении
...
big_int::big_int(const std::vector<unsigned int, pp_allocator<unsigned int> > &digits, bool sign) : _digits(digits),
    _sign(sign) {
    if (_digits.empty()) {
        _digits.push_back(0);
    }
...
big_int::big_int(std::vector<unsigned int, pp_allocator<unsigned int> > &&digits,
                 bool sign) noexcept : _digits(std::move(digits)), _sign(sign) {
    if (_digits.empty()) {
        _digits.push_back(0);
    }
    optimise(_digits);
...
big_int::big_int(const std::string &str_num, unsigned int base, pp_allocator<unsigned int> allocator) : _sign(true),
    _digits(allocator) {
    if (str_num.empty()) {
        _digits.push_back(0);
        return;
    ...
    while (tempstr.size() > 1 && tempstr[0] == '0') {
        tempstr = tempstr.substr(1);
    }
