...
TEST(positiveTests, test1) {
    ...
    std::unique_ptr<smart_mem_resource> subject(
        new allocator_boundary_tags(sizeof(int) * 70, nullptr, logger.get(),
    ...
    ASSERT_EQ(reinterpret_cast<int*>(reinterpret_cast<char*>(first_block + 10) + sizeof(size_t) + sizeof(void*) * 3),
              second_block);
    ASSERT_EQ(reinterpret_cast<int*>(reinterpret_cast<char*>(second_block + 10) + sizeof(size_t) + sizeof(void*) * 3),
              third_block);
    ...
    ASSERT_EQ(reinterpret_cast<int*>(reinterpret_cast<char*>(first_block + 10) + sizeof(size_t) + sizeof(void*) * 3),
              fourth_block);
    ASSERT_EQ(reinterpret_cast<int*>(reinterpret_cast<char*>(fourth_block + 1) + sizeof(size_t) + sizeof(void*) * 3),
              fifth_block);
...
TEST(positiveTests, test2) {
    ...
    std::unique_ptr<smart_mem_resource> allocator_instance(new allocator_boundary_tags(
        sizeof(unsigned char) * 3000, nullptr, logger_instance.get(), allocator_with_fit_mode::fit_mode::first_fit));
    ...
    {
        {
            .block_size = 1000 + sizeof(allocator_dbg_helper::block_size_t) + sizeof(
                              allocator_dbg_helper::block_pointer_t) * 3,
            .is_block_occupied = true
        },
        {
            .block_size = sizeof(allocator_dbg_helper::block_size_t) + sizeof(allocator_dbg_helper::block_pointer_t) *
                          3,
            .is_block_occupied = true
        },
        {
            .block_size = 3000 - (1000 + (sizeof(allocator_dbg_helper::block_size_t) + sizeof(
                                              allocator_dbg_helper::block_pointer_t) * 3) * 2),
...
TEST(falsePositiveTests, test1) {
    ...
    std::unique_ptr<smart_mem_resource> allocator_instance(
        new allocator_boundary_tags(3000, nullptr, logger_instance.get(),
                                    allocator_with_fit_mode::fit_mode::first_fit));
...
TEST(own, test1) {
    ...
    std::unique_ptr<smart_mem_resource> alloc(
        new allocator_boundary_tags(4000, nullptr, logger_instance.get(),
                                    allocator_with_fit_mode::fit_mode::first_fit));
    ...
    std::unique_ptr<smart_mem_resource> allocator(
        new allocator_boundary_tags(5000, nullptr, logger_instance.get(),
                                    allocator_with_fit_mode::fit_mode::first_fit));
