...
allocator_boundary_tags::~allocator_boundary_tags() {
    ...
    if (logger_instance == nullptr) return;
    ...
    try {
        ...
        size_t total_size = allocator_metadata_size + reinterpret_cast<size_t>(
...
allocator_boundary_tags::allocator_boundary_tags(allocator_boundary_tags &&other) noexcept : _trusted_memory(
...
allocator_boundary_tags &allocator_boundary_tags::operator=(allocator_boundary_tags &&other) noexcept {
    ...
    if (this != &other) {
        ...
        if (_trusted_memory) {
            try {
                /*
                 * переходим к поле space_size
                 * *reinterpret_cast<size_t *>(reinterpret_cast<unsigned char *>(_trusted_memory) +
                                                                sizeof(logger *) +
                                                                sizeof(memory_resource *) +
                                                                sizeof(allocator_with_fit_mode::fit_mode));
...
[[nodiscard]] void *allocator_boundary_tags::do_allocate_sm(size_t size) {
    ...
    const size_t block_size = size + occupied_block_metadata_size;
...
void allocator_boundary_tags::do_deallocate_sm(void *curent) {
    logger *logger = get_logger();
    ...
    void *next_block = *reinterpret_cast<void **>(reinterpret_cast<char *>(curent) + sizeof(size_t));
    void *prev_block = *reinterpret_cast<void **>(reinterpret_cast<char *>(curent) + sizeof(size_t) + sizeof(void *));
    if (prev_block) {
...
inline void allocator_boundary_tags::set_fit_mode(allocator_with_fit_mode::fit_mode mode) {
    auto *memory = reinterpret_cast<unsigned char *>(_trusted_memory);
    auto *mode_ptr = reinterpret_cast<allocator_with_fit_mode::fit_mode *>(
        memory + sizeof(logger *) + sizeof(memory_resource *));
    *mode_ptr = mode;
}
...
void *allocator_boundary_tags::boundary_iterator::operator*() const noexcept {
    return reinterpret_cast<void *>(reinterpret_cast<unsigned char *>(_occupied_ptr) + sizeof(size_t) + 3 * sizeof(
                                        size_t)); // Пропускаем все метаданные
}
...
void *allocator_boundary_tags::allocate_best_fit(size_t size) {
    ...
    if (best_pos)
        return allocate_in_hole(reinterpret_cast<char *>(best_pos), size, first_block_ptr, best_prev,
                                best_next, best_diff);
    return nullptr;
}
...
void *allocator_boundary_tags::allocate_worst_fit(size_t size) {
    ...
    if (worst_pos)
        return allocate_in_hole(reinterpret_cast<char *>(worst_pos), size, first_block_ptr, worst_prev,
                                worst_next, worst_size);
    return nullptr;
}
