...
// Функция формирует строку лога по заданному формату (_format)
std::string client_logger::make_format(const std::string &message, const severity sev) const {
    // это поток для сборки строки из частей
...
logger &client_logger::log(const std::string &message, const logger::severity severity) & {
    ...
    //Ищем потоки с важностью severity
    auto opened_stream = _output_streams.find(severity);
    if (opened_stream == _output_streams.end()) {
        return *this;
    }
    //Если нашли, то проверяем bool флаг (opened_stream->second.second) на вывод в консоль сообщения
    if (opened_stream->second.second) {
        std::cout << output << std::endl;
    }
    //для каждого stream из списка записываем в него message
...
//noexcept гарантирует, что конструктор не бросит исключений
