...
B_tree<tkey, tvalue, compare, t>::btree_iterator::operator++() {
	...
	//Случай 1: Если не лист
	if (!node->_pointers.empty()) {
		// Передвигаем указатель на следующий значение в ноде
		_path.top().second = _path.top().second + 1;
		// переходим в правый указатель родителя
		_path.push({ &node->_pointers[_index + 1], 0 });
		//укзатель на первый элемент ноды
	...
	// Если мы лист и не прошли всю ноду
...
B_tree<tkey, tvalue, compare, t>::btree_iterator::operator--() {
	...
	//TODO ПОМЕНЯТЬ ТРЕШ (В КОНСТАНТОНОМ ТОЖЕ)
	auto* node = *_path.top().first;
	//Случай 1: Обход ноды влево если можно
	if (_index > 0) {
		--_index;
		_path.top().second = _index;
		return *this;
	...
	if (node->_pointers.empty()) {
		while (!_path.empty()) {
			...
			_path.pop();
			//если путь пуст - мы удалили корень -> обход завершен
			if (_path.empty()) {
				_path = {};
				_index = 0;
				return *this;
...
size_t B_tree<tkey, tvalue, compare, t>::btree_const_iterator::depth() const noexcept {
	//подсчет идет с корня (поэтому -1)
	return _path.size() - 1;
}
...
B_tree<tkey, tvalue, compare, t>::insert(const tree_data_type& data) {
	...
	// Ищем уровень куда вставить
	while (true) {
		size_t idx = 0;
		//пока не вышли за границу ноды и пока текущий ключ < вставляемого
	...
	while (!path_stack.empty()) {
		...
		//если размер ноды превысил максимально количество значений
		//ее надо разделять

		size_t mid = t;
		auto mid_key = node->_keys[mid];
		//разделили на левое и правую часть
		btree_node* right = new btree_node();
		//обрезаем для правой ключи
		right->_keys.assign(
				node->_keys.begin() + mid + 1,
				node->_keys.end()
		);
		node->_keys.resize(mid);
		//если в ноде были указатели обрезаем и указатели для правого
		if (!node->_pointers.empty()) {
		...
		//есди родителя нет, значит выносим середину наверх и она становится корнем
		if (path_stack.empty()) {
			btree_node* new_root = new btree_node();
			new_root->_keys.push_back(mid_key);
			//подвязываем в левый указатель node, а в правый right
			new_root->_pointers.push_back(node);
			new_root->_pointers.push_back(right);
			_root = new_root;
			//родитель есть, значит середину наверх
		} else {
			auto [parent, ignored] = path_stack.top();

			//Вычисляем ключ который больше всего нам подходит
...
B_tree<tkey, tvalue, compare, t>::erase(const tkey& key) {
    ...
    std::function<bool(btree_node*, btree_node*, size_t, const tkey&)> remove_key;
    remove_key = [&](btree_node* node, btree_node* parent, size_t idx_in_parent, const tkey& k) -> bool {
        size_t idx = 0;
        ...
        if (idx < node->_keys.size() && !compare_keys(k, node->_keys[idx].first) && !compare_keys(node->_keys[idx].first, k)) {
            if (node->_pointers.empty()) {
                ...
                if (pred->_keys.size() >= t) {
                    // Find predecessor
                    while (!pred->_pointers.empty()) pred = pred->_pointers.back();
                    auto pred_pair = pred->_keys.back();
                    node->_keys[idx] = pred_pair;
                    remove_key(node->_pointers[idx], node, idx, pred_pair.first);
                } else {
                    btree_node* succ = node->_pointers[idx + 1];
                    if (succ->_keys.size() >= t) {
                        // Find successor
                        while (!succ->_pointers.empty()) succ = succ->_pointers.front();
                        auto succ_pair = succ->_keys.front();
                        node->_keys[idx] = succ_pair;
                        remove_key(node->_pointers[idx + 1], node, idx + 1, succ_pair.first);
                    } else {
                        // Merge pred, key, succ
                        merge(node, idx);
                        remove_key(node->_pointers[idx], node, idx, k);
                    }
                }
            }
            ...
            if (child->_keys.size() == minimum_keys_in_node) {
                // Try to fix underflow before descent
            ...
            remove_key(child, node, idx, k);
        }
        // After removal, check if node is underflow (except root)
    ...
    remove_key(_root, nullptr, 0, key);

    // If root is empty and has children, make child new root
