...
void B_tree_disk<tkey, tvalue, compare, t>::rebalance_node(
    ...
    size_t node_idx_in_pointers = 0;
    while (node_idx_in_pointers <= parent.size && parent.pointers[node_idx_in_pointers] != node.position_in_disk) {
        node_idx_in_pointers++;
    }

    // Стратегия 1: Попытаться заимствовать ключ у левого соседа
    // Если мы не самый левый
    if (node_idx_in_pointers > 0) {
        size_t left_sibling_pos = parent.pointers[node_idx_in_pointers - 1];
        ...
        if (left_sibling.size > minimum_keys_in_node) {
            ...
            // Вставляем родителя в начало текущего узла
            node.keys.insert(node.keys.begin(), parent.keys[node_idx_in_pointers - 1]);
            node.size++;

            // Перемещаем последний ключ из левого соседа в родителя
            parent.keys[node_idx_in_pointers - 1] = left_sibling.keys[left_sibling.size - 1];
    ...
    if (node_idx_in_pointers < parent.size) {
        size_t right_sibling_pos = parent.pointers[node_idx_in_pointers + 1];
        ...
        if (right_sibling.size > minimum_keys_in_node) {
            ...
            node.keys.push_back(parent.keys[node_idx_in_pointers]);
            node.size++;

            // Перемещаем первый ключ из правого соседа в родителя
            parent.keys[node_idx_in_pointers] = right_sibling.keys[0];
    ...
    if (node_idx_in_pointers > 0) {
        ...
        left_sibling.keys.push_back(parent.keys[node_idx_in_pointers - 1]);
        left_sibling.size++;
        ...
        parent.keys.erase(parent.keys.begin() + (node_idx_in_pointers - 1));
        parent.pointers.erase(parent.pointers.begin() + node_idx_in_pointers);
        parent.size--;
        disk_write(parent);
    ...
    if (node_idx_in_pointers <= parent.size) {
        ...
        parent.keys.erase(parent.keys.begin() + node_idx_in_pointers);
        parent.pointers.erase(parent.pointers.begin() + (node_idx_in_pointers + 1));
        parent.size--;
        disk_write(parent);
...
// Используется после сплита, левое поддерево уже подвязано, вставляем mid и right_tree
template<serializable tkey, serializable tvalue, compator<tkey> compare, std::size_t t>
...
B_tree_disk<tkey, tvalue, compare, t>::btree_disk_const_iterator::operator++() {
    ...
    if (!node._is_leaf) {
        ...
        // 2.1) сдвигаем активный указатель на следующие значения в ноде чтобы после вернутся уже к нему
