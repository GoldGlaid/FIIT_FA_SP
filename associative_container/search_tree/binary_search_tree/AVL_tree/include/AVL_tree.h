...
namespace __detail {
	...
	void bst_impl<tkey, tvalue, compare, AVL_TAG>::post_insert(binary_search_tree<tkey, tvalue, compare, AVL_TAG>& cont,
		...
		while (current) {
			...
			// у глав корня дизбаланс вправо
			if (balance > 1) {// Big left
				auto* right = static_cast<avl_node*>(current->right_subtree);
				// если в корня-правом дизбаланс слева
				...
				//меняем ссылку на current на теперь текущею ноду, которая стоит на месте current после перекрута
				current = static_cast<avl_node*>(subtree_ref);
				// у глав корня дизбаланс влево
			} else if (balance < -1) {// Big right
				auto* left = static_cast<avl_node*>(current->left_subtree);
				// если в корня-левом дизбаланс вправо
	...
	void bst_impl<tkey, tvalue, compare, AVL_TAG>::erase(
		...
		//случай: нету детей
		//зануляем связи у нашего родителя смотря кто мы
		if (!node->left_subtree && !node->right_subtree) {
			...
			// predecessor == приеемник наибольший узел в левом поддереве
			...
			//если премник не наш ребенок
			if (predecessor->parent != node) {
				//переподвязываем на родителя приемника его левого ребенка(правого не может быть по алгосу)
				predecessor->parent->right_subtree = predecessor->left_subtree;
				if (predecessor->left_subtree) {
					//переподвязываем на ребенка премника его родителя
					predecessor->left_subtree->parent = predecessor->parent;
				}

				balance_start = predecessor->parent;
			}
			//разветвление 1 если приемник сын удаляемого узла то ничего не делаем с левым поддеревом приемника
			// 2 иначе переподвязываем левое поддерево удаляемого узла к приемнику
			...
			//переподвешиваем правого ребенка удаляемого узла к приемнику
