...
void __detail::bst_impl<tkey, tvalue, compare, tag>::swap(binary_search_tree<tkey, tvalue, compare, tag> &lhs,
                                                          binary_search_tree<tkey, tvalue, compare, tag> &rhs)
    noexcept {
    using std::swap;
...
binary_search_tree<tkey, tvalue, compare, tag>::binary_search_tree(iterator begin, iterator end, const compare &cmp,
                                                                   pp_allocator<typename binary_search_tree<tkey, tvalue
                                                                       , compare, tag>::value_type> alloc,
                                                                   logger *logger) : compare(cmp), _root(nullptr),
    _logger(logger), _size(0), _allocator(alloc) {
...
explicit binary_search_tree(iterator begin, iterator end, const compare &cmp = compare(),
                            ...
                            logger *logger = nullptr) -> binary_search_tree<const typename std::iterator_traits<
    iterator>::value_type::first_type, typename std::iterator_traits<iterator>::value_type::second_type, compare>;

template<typename compare, typename U, std::ranges::forward_range Range>
explicit binary_search_tree(Range &&range, const compare &cmp = compare(),
                            pp_allocator<U> alloc = pp_allocator<U>(),
                            logger *logger = nullptr) -> binary_search_tree<const typename std::iterator_traits<typename
    std::ranges::iterator_t<Range> >::value_type::first_type, typename std::iterator_traits<typename
    std::ranges::iterator_t<Range> >::value_type::second_type, compare>;
...
binary_search_tree<tkey, tvalue, compare,
    tag>::node::node(node *parent, Args &&... args) : data(std::forward<Args>(args)...), parent(parent),
                                                      left_subtree(nullptr), right_subtree(nullptr) {
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::prefix_iterator::prefix_iterator(node *data, node *backup) : _data(data), _backup(backup) {
}
...
binary_search_tree<tkey, tvalue, compare, tag>::prefix_iterator::operator++() & noexcept {
    ...
    if (_data->left_subtree != nullptr) {
        ...
        // пока мы не корень и (пока мы не правый лист или пока мы не дерево без правого поддерева) = поднимаемся выше
...
binary_search_tree<tkey, tvalue, compare, tag>::prefix_iterator::operator--() & noexcept {
    ...
    if (parent == nullptr) {
        ...
        // Если у родителя нет левого поддерева
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::prefix_const_iterator::prefix_const_iterator(const node *data, const node *backup) : _base(
    const_cast<node *>(data), const_cast<node *>(backup)) {
}

template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::prefix_const_iterator::prefix_const_iterator(
    const prefix_iterator &other) noexcept : _base(other) {
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::prefix_reverse_iterator::prefix_reverse_iterator(node *data, node *backup) : _base(data, backup) {
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::prefix_reverse_iterator::operator binary_search_tree<tkey, tvalue,
    compare, tag>::prefix_iterator() const noexcept {
    auto copy = _base;
    ++copy;
    return copy;
}
...
binary_search_tree<tkey, tvalue, compare, tag>::prefix_const_reverse_iterator::prefix_const_reverse_iterator(
    const node *data, const node *backup) : _base(data, backup) {
}

template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::prefix_const_reverse_iterator::prefix_const_reverse_iterator(
    const prefix_const_iterator &it) noexcept : _base(it._base) {
    --_base;
}

template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::prefix_const_reverse_iterator::operator binary_search_tree<tkey, tvalue,
    compare, tag>::prefix_const_iterator() const noexcept {
    auto copy = _base;
    ++copy;
    return copy;
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::infix_iterator::infix_iterator(node *data, node *backup) : _data(data), _backup(backup) {
}
...
// чтобы сделать полный обход - начни с самого левого узла
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
...
binary_search_tree<tkey, tvalue, compare, tag>::infix_iterator::operator--() & noexcept {
    ...
    _backup = _data;
    //ищем максимальный в левом поддереве
    if (_data->left_subtree != nullptr) {
        _data = _data->left_subtree;
        //до конца вправо там самый максимальный
        ...
        while (parent != nullptr && _data == parent->left_subtree) {
            // поднимаемся до тех пор пока мы не правый или не корень
            _data = parent;
            parent = parent->parent;
        }
        _data = parent;
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::infix_const_iterator::infix_const_iterator(const node *data, const node *backup) : _base(
    const_cast<node *>(data), const_cast<node *>(backup)) {
}

template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::infix_const_iterator::infix_const_iterator(
    const infix_iterator &it) noexcept : _base(it) {
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::infix_reverse_iterator::infix_reverse_iterator(node *data, node *backup) : _base(data, backup) {
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::infix_reverse_iterator::operator binary_search_tree<tkey, tvalue,
    compare, tag>::infix_iterator() const noexcept {
    auto copy = _base;
    ++copy;
    return copy;
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::infix_const_reverse_iterator::infix_const_reverse_iterator(
    const infix_const_iterator &it) noexcept : _base(it) {
    --_base;
}

template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::infix_const_reverse_iterator::operator binary_search_tree<tkey, tvalue,
    compare, tag>::infix_const_iterator() const noexcept {
    auto copy = _base;
    ++copy;
    return copy;
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::postfix_iterator::postfix_iterator(node *data, node *backup) : _data(data), _backup(backup) {
}
...
binary_search_tree<tkey, tvalue, compare, tag>::postfix_iterator::operator--() & noexcept {
    ...
    // листовой узел
        ...
        // Поднимаемся, пока не найдём родителя, у которого текущий узел - левый потомок
        while (parent != nullptr &&
               (_data == parent->left_subtree || parent->left_subtree == nullptr)) {
            _data = parent;
            parent = parent->parent;
        }
        if (parent == nullptr) {
            _data = nullptr; // Достигли корня
        } else {
            // Переходим в левое поддерево родителя
            _data = parent->left_subtree;

            // Идём в самый глубокий правый узел этого поддерева
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::postfix_const_iterator::postfix_const_iterator(const node *data, const node *backup) : _base(
    const_cast<node *>(data), const_cast<node *>(backup)) {
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::postfix_reverse_iterator::postfix_reverse_iterator(node *data, node *backup) : _base(data, backup) {
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::postfix_reverse_iterator::operator binary_search_tree<tkey, tvalue,
    compare, tag>::postfix_iterator() const noexcept {
    auto copy = _base;
    ++copy;
    return copy;
}
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare,
    tag>::postfix_const_reverse_iterator::postfix_const_reverse_iterator(const node *data, const node *backup) : _base(
    const_cast<node *>(data), const_cast<node *>(backup)) {
}

template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::postfix_const_reverse_iterator::postfix_const_reverse_iterator(
    const postfix_const_iterator &it) noexcept : _base(it) {
    --_base;
}

template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
binary_search_tree<tkey, tvalue, compare, tag>::postfix_const_reverse_iterator::operator binary_search_tree<tkey, tvalue
    , compare, tag>::postfix_const_iterator() const noexcept {
    auto copy = _base;
    ++copy;
    return copy;
}
...
tvalue &binary_search_tree<tkey, tvalue, compare, tag>::at(const tkey &key) {
    ...
    while (current != nullptr) {
        //a < b == true [key < current->data.first]
...
tvalue &binary_search_tree<tkey, tvalue, compare, tag>::operator[](const tkey &key) {
    auto result = insert_or_assign({key, tvalue{}});
    return result._data->data.second;
}

// оператор[] для rvalue
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
tvalue &binary_search_tree<tkey, tvalue, compare, tag>::operator[](tkey &&key) {
    auto result = insert_or_assign({std::move(key), tvalue{}});
    return result._data->data.second;
}
...
binary_search_tree<tkey, tvalue, compare, tag>::insert(const value_type &value) {
    ...
    while (current != nullptr) {
        ...
        if (compare_keys(value.first, current->data.first)) {
            ...
            // возвращаем итератор на сущеввтующий узел, false = вставка не выполнена
...
template<typename tkey, typename tvalue, compator<tkey> compare, typename tag>
template<class... Args>
std::pair<typename binary_search_tree<tkey, tvalue, compare, tag>::infix_iterator, bool> binary_search_tree<tkey, tvalue
    , compare, tag>::emplace(Args &&... args) {
...
void binary_search_tree<tkey, tvalue, compare, tag>::insert_or_assign(InputIt first, InputIt last) {
    ...
    //TODO ИЗМЕНИЛ
    for (auto it = first; it != last; ++it) {
        insert_or_assign(std::forward<decltype (*it)>(*it));
    }
...
bool binary_search_tree<tkey, tvalue, compare, tag>::contains(const tkey &key) const {
    node *current = _root;
    while (current != nullptr) {
        //TODO изменил compare.operator() -> compare_keys и дальше
        if (compare_keys(key, current->data.first)) {
            current = current->left_subtree;
        } else if (compare_keys(current->data.first, key)) {
            current = current->right_subtree;
...
void binary_search_tree<tkey, tvalue, compare, tag>::small_left_rotation(node *&subtree_root) noexcept {
    ...
    // переподвешиваем левого ребенка B к старому root
    subtree_root->right_subtree = new_root->left_subtree;

    // меняем у ребенка которого переподвесили родителя
    ...
    //переподвешиваем А к B
    new_root->left_subtree = subtree_root;
    //меняйем родителей у корней
    ...
    void bst_impl<tkey, tvalue, compare, tag>::erase(binary_search_tree<tkey, tvalue, compare, tag> &cont,
        ...
        if (!has_left && !has_right) {
            ...
            *node_ptr = child;
            ...
            // ЛОГИКА ДЕТЕЙ
            //к родителю target подвешиваем детей target
            if (temp_subtree->parent->left_subtree == temp_subtree) {
                temp_subtree->parent->left_subtree = temp_subtree->left_subtree;
            } else {
                temp_subtree->parent->right_subtree = temp_subtree->left_subtree;
            }
            //к правому ребенку target подвешиваем родителя target
            ...
            //ЛОГИКА РОДИТЕЛЕЙ
            //к левому поддереву target подвешиваем левое поддерево node
            temp_subtree->left_subtree = node->left_subtree;

            //к левому поддереву node подвешиваем родителя target
            if (temp_subtree->left_subtree) {
                temp_subtree->left_subtree->parent = temp_subtree;
            }
            //к правому поддереву target подвешиваем правое поддерево node
            temp_subtree->right_subtree = node->right_subtree;
            if (temp_subtree->right_subtree) {
                //к правому поддереву node подвешиваем родителя target
                temp_subtree->right_subtree->parent = temp_subtree;
            }
            //к target подвешиваем родителя node
            temp_subtree->parent = node->parent;
            if (node->parent) {
                if (node->parent->left_subtree == node) {
                    //к левому поддереву родителю node подвешиваем target
                    node->parent->left_subtree = temp_subtree;
                } else {
                    //к правому поддереву родителю node подвешиваем target
